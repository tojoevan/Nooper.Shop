...
class User {
	...
	/**
	 * public boolean function subscribe(void)
	 */
	public function subscribe($datas): bool {
		$mysql = new Mysql();
		$record = $mysql->memory('user_subscribe_record')->insert($datas);
		return $record > 0 ? true : false;
	}
	
	/**
	 * public integer function add_text_message(array $datas)
	 */
	public function add_text_message(array $datas): int {
		$mysql = new Mysql();
		return $mysql->memory('user_text_message')->add($datas);
	}
	
	/**
	 */
	public function unsubscribe($datas): bool {
		$mysql = new Mysql();
		$record = $mysql->memory('user_unsubscribe_record')->insert($datas);
		return $record > 0 ? true : false;
	}
